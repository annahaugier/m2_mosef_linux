...
app.layout = html.Div(children=[
    html.Div(children=[
        html.H3('Taux de criminalité par habitant', style={'margin-left': '20px', 'flex': '1', 'font-family': 'Arial, Helvetica, sans-serif'}),
        html.Div(children=[slider], style={'flex': '4'}),
    ], style={'height': '5vh', 'display': 'flex', 'justify-content': 'space-evenly', 'align-items': 'center'}),
    dl.Map(children=[
        dl.TileLayer(),
        geojson,
        colorbar,
...
@app.callback(Output('info', 'children'), Input('geojson', 'hoverData'), Input('selected_year', 'data'))
def info_hover(feature, year):
    if year is None:
        year = '2016'
    return get_info(feature, json_data, year)

@app.callback(Output('geojson', 'hideout'), Input('selected_year', 'data'))
def update_hideout(value):
    if value is None:
        value = '2016'
    return get_hideout(json_data, str(value))

@app.callback(Output('selected_year', 'data'), Input('slider', 'value'))
